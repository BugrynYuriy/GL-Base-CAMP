...
int BinSearch(PassportSiries *arr, int count, PassportSiries key) 
{
  ...
  while (l <= u) {
	...
	if (arr[m].hash == key.hash) return m;  // якщо середній елемет дорівнюємо шуканий , повертаємо його індекс 
	if (arr[m].hash < key.hash) l = m + (int)1;	// якщо середній елемент менший шуканого, початок переносимо  на середній + 1
	if (arr[m].hash > key.hash) u = m - (int)1;	// якщо середній елемент більший шуканого, кінець переносимо   на середній - 1
  }
...
PassportSiries * deleteElement (PassportSiries *arr, int &size, PassportSiries element)
{
	...
	index = BinSearch(arr,size,element); // поршук елементу у масивы
...
int main ()
{
	...
	while(true) 
	{
		...
		if(isValidPassportSiries(key)&&key.siries[8]=='\0') // перевірямо серію паспорту на валідність
		{
	...
	while(true)
	{
		...
		if(isValidPassportSiries(key)&&key.siries[8]=='\0') // перевірямо серію паспорту на валідність
		{
			index=BinSearch(arr,size,key); // використовуємо функцію пошуку
